...
![O eCatalog como ele aparece para os usuários. A) Página de abertura do eCatalog. B)Catálogo eletrônico convertido na página 2.](/help/assets/ec_cat_viewer_popup.png)

Para criar um eCatalog, você normalmente usa arquivos de PDF de alta resolução criados no Adobe Acrobat ou em outro programa de impressão, mas também pode criar um eCatalog a partir de arquivos de imagem.
...
>[!NOTE]
>
>Se você for um usuário do modo Dynamic Media - Scene7 e quiser usar Catálogos eletrônicos, edite a `pdfbrochure` em CRXDE Lite. Para fazer isso, no Adobe Experience Manager, acesse **[!UICONTROL Tools]** > **[!UICONTROL General]** > **[!UICONTROL CRXDE Lite]**. Na árvore de navegação do painel esquerdo, navegue até `/conf/global/settings/cloudconfigs/dmscene7/jcr:content/mimeTypes/application_pdf`.
>
>No painel inferior direito, no **[!UICONTROL Properties]** selecione a guia `jobParam` linha. Defina o valor para `pdfbrochure` from `false` para `true`. Em `pdfbrochure=true`
>
...
Na barra Navegação global, selecione **[!UICONTROL Upload]** para iniciar o upload de arquivos ou imagens do PDF para seu eCatalog. Você pode fazer upload de arquivos do desktop ou via FTP; O FTP é recomendado se você estiver carregando muitos arquivos ou arquivos com mais de 100 MB.
...
Consulte [Fazer upload dos arquivos do PDF](uploading-pdf-files.md#uploading_the_pdf_files).
...
Na página Catálogo eletrônico , na **[!UICONTROL Order Pages]** selecione uma opção Layout : **[!UICONTROL 1 Up]**, **[!UICONTROL 2 Up]** ou **[!UICONTROL Custom]**. Você pode reorganizar páginas ou páginas espelhadas arrastando-as ou, em um eCatalog grande, escolhendo um nome de página no menu Mover para.
...
Selecionar **[!UICONTROL Save]**, insira um nome para seu eCatalog, escolha uma pasta do Adobe Dynamic Media Classic para armazená-lo e selecione **[!UICONTROL Save]**. Sempre que alterar a ordem da página ou editar seu eCatalog, salve as alterações clicando em **[!UICONTROL Save]**.

Consulte [Criar um catálogo eletrônico](creating-ecatalog.md).
...
Para criar um Mapa de imagem, abra a tela Catálogo eletrônico . Em seguida, vá para a **[!UICONTROL Map Pages]** da tela Catálogo eletrônico e enquadre o mapa com a ferramenta Mapa de imagem em retângulo ou com a ferramenta Mapa de imagem em polígono. É possível ajustar a posição e o tamanho dos mapas de imagem arrastando as bordas do mapa com a ferramenta Deslocar.
...
Consulte [Gerenciar o conteúdo do painel Informações em catálogos eletrônicos](/help/info-panel-content-ecatalog.md).
...
Para criar uma Predefinição do Visualizador de Catálogo Eletrônico, na barra Navegação Global, selecione **[!UICONTROL Setup]** e escolha **[!UICONTROL Viewer Presets]**. Selecionar **[!UICONTROL Add]**, escolha uma plataforma e selecione **[!UICONTROL eCatalog]** > **[!UICONTROL Viewer]**.

Consulte [Configurar predefinições do visualizador de catálogo eletrônico](setting-ecatalog-viewer-presets.md#setting-up-ecatalog-viewer-presets).
...
Consulte [Visualizar eCatalogs no Visualizador de Catálogo Eletrônico](previewing-ecatalogs-ecatalog-viewer.md#previewing-ecatalogs-in-the-ecatalog-viewer).
