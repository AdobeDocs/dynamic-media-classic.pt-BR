...
**Adobe Dynamic Media Classic** UserCan acessar empresas às quais foram atribuídas; não possa desempenhar quaisquer funções administrativas.

**Adobe Dynamic Media Classic Empresa** AdminPode visualização e administrar somente suas próprias empresas. Um administrador de Empresa também pode executar todas as funções administrativas, incluindo a adição de administradores e usuários. Um administrador da Empresa pode adicionar um usuário às contas de administrador da empresa do DMC. (Essa função é a função padrão do usuário.)
...
1. Clique em **Configuração** > **Configuração da Aplicação** > A **Configuração da Administração** > **Administração do Utilizador**.
1. Selecione um usuário na lista e clique em **Editar**.
1. Desmarque Válido.
1. Clique em **Salvar**.

### Ativando ou desativando usuários {#activating-or-deactivating-users}
...
1. Vá para **Configuração** > **Configuração de Aplicação**> A **Configuração de Administração** > **Administração de Utilizadores**.
...
### Filtrando e classificando a lista do usuário {#filtering-and-sorting-the-user-list}
...
**Filtrar por** grupoSelecione o menu Por grupo e escolha uma opção para restringir a lista aos usuários em um grupo.
...
**Filtrar** usuários inválidosDesmarque a opção Incluir inválidos. Os resultados da pesquisa exibem somente os usuários que estão no sistema. Usuários inválidos foram excluídos do sistema e das contas que você administra.

**Classificar por** cabeçalho de colunaClique em um cabeçalho para classificar todos os usuários por status, em ordem alfabética por nome, sobrenome ou email, por função de usuário ou por status válido/inválido.
...
**Image** RequestRequests para imagens.

**Miniaturas** Solicitações para amostras ou imagens alternativas em visualizadores.

**Mascarar** Solicitações para imagens que retornam máscaras em escala de cinza.

**Solicitações de** RequestImage do mosaico do visualizador carregadas por um visualizador.

**Solicitações de renderização** RequestImage do objeto Vnt que retornam uma imagem com objetos especificados nas vinhetas solicitadas.

**Solicitação de renderização de Informações do Vnt** RequestImage que retorna informações sobre as vinhetas solicitadas.
...
**Exibição de dados em um gráfico ou** grade de dadosClique na opção Visualização de gráfico para visualização de dados em um gráfico; clique na opção Visualização de dados para visualização de dados em uma grade de dados.
...
**Especificação de um** período de tempoEscolha as opções para definir um período de tempo para seu relatório e clique em Atualizar depois de definir um período de tempo:
...
**Período de tempo personalizadoNo menu Relatório predefinido, escolha Personalizado.** Em seguida, escolha uma data no menu Mês do Start (ou Data do Start) e uma data no menu # de Meses (ou # ou Dias). Para Relatórios de conteúdo de domínio e vídeo, você pode escolher um start específico e uma data de término para capturar informações do relatório.
...
**Imagem** IDID para a imagem ofensiva.

**** TempoO intervalo de tempo da primeira vez que o erro foi relatado para a última vez que o erro foi relatado, nas últimas 24 horas.

**** ContagemO número de erros reportados na imagem.

**** RespostaA mensagem de erro específica. Os erros são 4xx ou 5xx.

**** URLsLista o URL para a imagem no Scene 7.

**** ReferenciadorEspecifica o URL do site de onde a solicitação inicial veio. A quem indicou pode ser qualquer site que tenha um link para a imagem.
