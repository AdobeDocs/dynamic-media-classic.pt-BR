...
Para obter mais informações, consulte o PDF [Aviso de publicação](https://marketing.adobe.com/resources/help/en_US/s7/rendering-instant-publish-notification.pdf)instantânea.

Use estas técnicas para marcar ativos para publicação:

* **Publicar depois de carregar** Na página Carregar, perto da parte inferior, selecione Publicar depois de carregar. O padrão é um estado selecionado.
...
1. Na caixa de diálogo Publicar, escolha se deseja um trabalho de publicação único ou recorrente.
   ...
   Consulte [Criação de um trabalho](publishing-files.md#creating_a_one_time_publish_job) de publicação única e [Criação de um trabalho](publishing-files.md#creating_a_recurring_publish_job)de publicação recorrente.

1. Insira um nome de tarefa.
1. Opcionalmente, exiba as opções Avançadas e escolha essas opções.

   Consulte Opções [de publicação](publishing-files.md#advanced_publish_options)avançadas.
...
**Notificação** HTTPInsira um URL para acionar trabalhos de publicação subsequentes.

Consulte [Usar um trabalho de upload ou publicação como disparador](checking-job-files.md#using_an_upload_or_publish_job_as_a_trigger).)
