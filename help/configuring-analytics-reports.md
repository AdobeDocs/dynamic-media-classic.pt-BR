...
seo-title: Configuração de relatórios da Adobe Analytics
description: nulo
...
| JOGAR | **X** (Vídeo) | Em visualizadores de vídeo, quando um usuário clica em Reproduzir para start reproduzindo um vídeo.<br><br>**Observação:** Se estiver usando o relatórios de vídeo baseado em pulsação de Adobe Analytics, não será necessário mapear nenhuma variável para esse evento do visualizador ao configurar o Adobe Analytics no Dynamic Media Classic. O Video Heartbeat funciona com os visualizadores de vídeo HTML5 e de mídia mista do Dynamic Media Classic prontos para uso. O player de vídeo gera dados de rastreamento para exibição nos Relatórios de vídeo da Adobe Analytics. Consulte [Ativação dos relatórios](enabling-analytics-video-reports.md)de vídeo da Adobe Analytics. |
...
| ZOOM | **X** (eCatalog, SpinSet, Zoom) | Não rastreado pela Adobe Analytics.<br> |
| PAN | **X** (eCatalog, SpinSet, Zoom) | Não rastreado pela Adobe Analytics.<br> |
| SPIN | **X** (SpinSet) | Não rastreado pela Adobe Analytics.<br> |


### Variáveis do Dynamic Media Classic {#scene-variables}

Para cada evento do visualizador na tela Configuração do Adobe Analytics, escolha uma variável Adobe Analytics e uma variável ** Dynamic Media Classic. As variáveis do Dynamic Media Classic representam dados que você pode obter para um relatório. Por exemplo, a variável listas palavras-chave usadas em pesquisas do eCatalog. `searchTerm`
