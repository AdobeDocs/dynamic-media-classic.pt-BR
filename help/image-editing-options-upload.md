...
Essas opções estão na página Upload , em Opções de edição de imagem.
...
Para criar uma máscara para a imagem com base em suas informações de traçado de recorte, clique em **[!UICONTROL Create Mask From Clipping Path]**. Essa opção se aplica a imagens criadas com aplicativos de edição de imagens nas quais um traçado de recorte foi criado.
