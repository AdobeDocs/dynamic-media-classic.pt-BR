...
* **[!UICONTROL Show Projects]** - Determina se os Projetos estão disponíveis como um meio de organizar os ativos do Adobe Dynamic Media Classic. Consulte [Organize seu trabalho com Projetos](/help/organizing-projects.md).
...
* **[!UICONTROL Default Upload Options]** - Abre a caixa de diálogo Fazer upload das opções de trabalho, onde é possível especificar as opções de upload padrão. Para obter informações sobre essas opções, consulte [Opções de upload](/help/uploading-files.md#upload_options).
...
* **[!UICONTROL Trash Can Clean Up Warnings]** - Os ativos na Lixeira são removidos automaticamente em sete dias. Selecione &quot;Enviar emails antes que os itens da lixeira sejam excluídos automaticamente&quot; se desejar que as notificações sejam enviadas aos administradores da empresa quando os ativos contidos na lixeira estiverem a quatro dias de serem excluídos permanentemente. Consulte [Gerenciar a pasta Lixeira](/help/trash-folder.md).
...
Uma predefinição de codificação que funciona com qualquer taxa de proporção para permitir que você crie vídeos para levar a dispositivos móveis, tablets e computadores de mesa. Os vídeos de origem carregados que são codificados usando essa predefinição (o padrão e uma prática recomendada) são definidos com uma altura fixa, enquanto a largura é dimensionada automaticamente para preservar a proporção do vídeo.
...
Não há número mínimo ou máximo de linhas ou colunas que você deve ter em um conjunto de rotação multieixo.
