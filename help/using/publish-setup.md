...
>[!NOTE]
>
>As páginas de configuração de publicação devem ser usadas por desenvolvedores de sites e profissionais de programação experientes. O Adobe Dynamic Media Classic presume que os usuários que alteram as configurações nessas páginas estão familiarizados com o Adobe Dynamic Media Classic, os padrões e convenções de protocolo HTTP e a tecnologia básica de geração de imagens.
...
* **[!UICONTROL Localization Support]**: essas configurações permitem gerenciar vários atributos de localidade. Eles também permitem especificar uma string do mapa de localidade. Isso permite que você defina os idiomas que deseja suportar. Elas se aplicam às várias dicas de ferramentas em Visualizadores.
...
IS suporta strings ASCII imprimíveis arbitrárias. O comando `locale=` se aplica globalmente a toda a solicitação, incluindo solicitações aninhadas, modelos e camadas. Não há suporte para várias localidades por solicitação, como uma localidade diferente para cada camada. No entanto, é concebível permitir substituições explícitas em solicitações aninhadas.

Se `locale=` não for especificado, `attribute::DefaultLocale` será passado para os mecanismos de tradução. Validação de entrada limitada aplicada ao valor `locale=`. São permitidos valores `locale=` vazios. Como `locale=` tem um escopo global, o catálogo principal fornece `attribute::DefaultLocale` para toda a solicitação.
...
* Oferece suporte a todo o conteúdo gerenciado por catálogo que precise de localização, incluindo imagens, conjuntos, vinhetas, materiais e configurações.
...
Use um locSuffix dedicado para localidades desconhecidas e force para a imagem padrão se nenhum `_U` existir, como no seguinte:
...
Usando o primeiro exemplo como base, as imagens para todas as linguagens têm os sufixos `_1`, `_2` ou `_3`. Imagens específicas para localidades francesas têm os sufixos `_22` ou `_23`. E as imagens específicas para localidades alemãs têm os sufixos `_470` ou `_480`.
...
* A localização é limitada a chamadas de ativos baseadas em ID e não pode ser usada em chamadas de ativos baseadas em caminho. Portanto, ao chamar vídeos com um local, eles devem ser chamados como company/assetID sem um caminho completo para o vídeo. Você não pode usar `RTMP` com localização porque esse método deve ser usado somente com chamadas de vídeo baseadas em caminho.
* Não é possível usar um Conjunto de mídias mistas que contenha um único vídeo quando o localeMap está ativo; caso contrário, a chamada para o conteúdo do conjunto falhará. Para resolver esse problema, adicione um único vídeo a um Conjunto de vídeos adaptados. Em seguida, adicione o Conjunto de vídeos adaptados a um Conjunto de mídias mistas.
...
* **[!UICONTROL Catalog Management]**: essas configurações determinam como o Adobe Dynamic Media Classic e o arquivo de catálogo interagem. As chamadas de URL do servidor de renderização do Adobe Dynamic Media Classic são feitas para o catálogo, que, por sua vez, entrega imagens do servidor. Altere essas configurações somente com a assistência de um membro da equipe de suporte da Adobe Dynamic Media Classic.
